...
update_a <- function(Y,X_pos,Y_neg,Y_hk,Y_reg,
...
update_a2 <- function(Y,X_pos,Y_neg,
                     cc,b,d_pos,d_neg,mu_a,
                     sig2_a,sig2_e,sig2_n,P,N,n_samples){
  sigma2 <-  1/(1/sig2_a + (P)/sig2_e+N/sig2_n)
  A1 <- colSums(Y_neg - cc*matrix(b,N,n_samples,byrow=T) - matrix(d_neg,N,n_samples))
  A2 <- colSums(Y-matrix(X_pos[,2],P,1)%*%matrix(b,1,n_samples) - 
                  matrix(d_pos,P,n_samples))
  mu <- sigma2*(mu_a/sig2_a + (A2)/sig2_e + A1/sig2_n)
  rnorm(n_samples,mu,sqrt(sigma2))
}



update_b <- function(Y,X_pos,Y_neg,Y_hk,Y_reg,
...
update_b2 <- function(Y,X_pos,Y_neg,
                     cc,a,d_pos,d_neg,d_hk,d_reg,mu_b,
                     sig2_b,sig2_e,sig2_n,P,N,n_samples){
  sigma2 <-1/(1/sig2_b + 
                (sum(X_pos[,2]^2))/sig2_e
              +N*(cc^2)/sig2_n)
  B1 <- matrix(cc,1,N)%*%(Y_neg-matrix(a,N,n_samples,byrow=T) - matrix(d_neg,N,n_samples))
  B2 <- matrix(X_pos[,2],1,P)%*%(Y-matrix(a,P,n_samples,byrow=T) - 
                                   matrix(d_pos,P,n_samples))
  mu <- sigma2*(mu_b/sig2_b + (B2)/sig2_e + B1/sig2_n)
  rnorm(n_samples,mu,sqrt(sigma2))
} ##done


update_cc <- function(Y_neg,a,b,d_neg,sig2_n,L=-10,U=10,N,n_samples){
...
update_phi <- function(Y_hk,Y_reg,a,b,d_hk,d_reg,kappa_hk,kappa_reg,
                       sig2_e,L=-10,U=10,H,R,n_samples){
  sigma2 <- sig2_e/((H+R)*b^2)
  P1 <- colSums(Y_hk-matrix(a,H,n_samples,byrow=T)-matrix(b,H,n_samples,byrow=T)*kappa_hk-matrix(d_hk,H,n_samples))
  P2 <- colSums(Y_reg-matrix(a,R,n_samples,byrow=T)-matrix(b,R,n_samples,byrow=T)*kappa_reg-matrix(d_reg,R,n_samples))
  mu <- (P1+P2)/((H+R)*b)
  rtruncnorm(1,L,U,mu,sqrt(sigma2))
}

update_phi2 <- function(Y_fixed){
  colMeans(Y) - mean(Y)
}

update_lambda <- function(Y){
  rowMeans(Y)
}

###Continue Checks here 10:26 AM 3/8/22 ####
update_kappa <- function(Y_dat,a,b,phi,d,lambda,
...
update_lambda <- function(kappa,sig2_kappa,L=-10,U=10,n_genes,n_samples){
  sig2<-sig2_kappa/n_samples
  mu<-rowMeans(kappa)
  rtruncnorm(n_genes,L,U,mu,sqrt(sig2))
}


update_dneg <- function(Y_neg,cc,a,b,d_neg,sig2_dn,sig2_n,N,n_samples){
  sigma2 <- 1/(n_samples/sig2_n + 1/sig2_dn)
  mu<-sigma2*rowSums(Y_neg-matrix(a,N,n_samples,byrow=T)-matrix(b,N,n_samples,byrow=T)*cc)/sig2_n
  rnorm(N,mu,sqrt(sigma2))
}

update_dneg2 <- function(Y_neg,cc,a,b,d_neg,sig2_dn,sig2_n,n_samples,N,j){
...
update_dpos <- function(Y_pos,X_pos,a,b,d_pos,sig2_d,sig2_e,P,n_samples){
  sigma2 <- 1/(n_samples/sig2_e + 1/sig2_d)
  mu<-sigma2*rowSums(Y_pos-matrix(a,P,n_samples,byrow=T)-
                       matrix(X_pos[,2],P,1)%*%matrix(b,1,n_samples))/sig2_e
  rnorm(6,mu,sqrt(sigma2))
}

update_dpos2 <- function(Y,X_pos,uj,a,b,d_pos,sig2_d,sig2_e,n_samples,j){
  dj <- sum_except2(j,d_pos[1:4])
  dxj <- sum_except2(j,(d_pos*X_pos[,2])[1:4])
  sigma2 <- ((n_samples*(1+uj[j]^2+(1+uj[j])^2)/sig2_e)+(1+uj[j]^2+(1+uj[j])^2)/sig2_d)^(-1)
  Dj <-  sum(Y[j,] - a - X_pos[j,2]*b)
  D5 <- uj[j]*(sum(Y[5,]-a-b*X_pos[5,2] - (X_pos[6,2]*dj)-dxj)/(X_pos[5,2]-X_pos[6,2]))
  D6 <- (1+uj[j])*sum(Y[6,]-a-b*X_pos[6,2]+dxj + ((X_pos[6,2]*dj)-dxj)/(X_pos[5,2]-X_pos[6,2]))
  mu <- sigma2*((Dj+D5-D6)/sig2_e)
  rnorm(1,mu,sqrt(sigma2))
}


get_d5 <- function(X_pos,d_pos){
  (sum((d_pos*X_pos[,2])[1:4]) - sum(d_pos[1:4])*X_pos[6,2])/(X_pos[6,2]-X_pos[5,2])
}

get_d6 <- function(d_pos){
  -sum(d_pos[1:5])
}

update_dpos3 <- function(Y,X_pos,a,b,sig2_d,sig2_e,P,n_samples,Sig_uns,D5,D6){
...
update_dhkreg <- function(Y,a,b,phi,kappa,sig2_e,sig2_d,n_genes,n_samples){
  sigma2 <- 1/(n_samples/sig2_e + 1/sig2_d)
...
update_mu <- function(z,sig2_z,prior_mean,prior_var,L=-10,U=10,n_samples){
...
update_sig2_abd <- function(z,mu_z,n,eps=.01){
  alpha <- eps+n/2
  beta <- eps + sum((z-mu_z)^2)/2
  1/rgamma(1,alpha,beta)
}

update_sig2_n <- function(Y_neg,a,b,cc,d_neg,n_genes,n_samples,eps=.01){
...
update_sig2_e <- function(Y_pos,X_pos,Y_hk,Y_reg,a,b,phi,kappa_hk,kappa_reg,
...
update_sig2_kappa <- function(kappa,lambda,n_genes,n_samples,eps=.01){
  alpha <- eps + (n_genes*n_samples)/2
