...
to perform meta analysis on NanoString nCounter gene expression data. We will use the example data provided under the `data/` folder for demonstration. 
...
```{r curation}
df = curate_data(dataset=df)
# Or 
# df = curate_data(dataset='../data/meta_analysis_data.csv')
head(df, n=12)
```
...
```{r regression}
results = find_regression_coefs(df=df)
head(results$df)
head(results$coeffs2)
```
Compared with the previous output, we can see that a *residual* column has been added to the *df* variable. Also, we have created a new dataframe containing all the regression coefficients. 
...
```
In our paper, we ran 4 MCMC chains in parallel of 12,000 samples. 
