...
## Fixed Values ####
MetaNorm <- function(dat, pos_conc = log10(c(128, 32, 8, 2, 0.5, 0.125)),
                          M=15000, mm = 3, seed=0){
  ...
  ##Begin Loop ####
  for (i in 1:M){
    Results$a[i,] <- a <- update_a(Y_pos,X_pos,Y_neg,Y_hk,Y_reg,
                                   cc,b,d_pos,d_neg,d_hk,d_reg,mu_a,phi,kappa_hk,kappa_reg,
                                   sig2_a,sig2_e,sig2_n,n_pos,n_neg,n_hk,n_reg,n_samples)
    Results$b[i,] <- b <- update_b(Y_pos,X_pos,Y_neg,Y_hk,Y_reg,
                                   cc,a,d_pos,d_neg,d_hk,d_reg,mu_b,phi,kappa_hk,kappa_reg,
                                   sig2_b,sig2_e,sig2_n,n_pos,n_neg,n_hk,n_reg,n_samples)
    Results$cc[i] <- cc <- update_cc(Y_neg,a,b,d_neg,sig2_n,L=-6,U=-1,n_neg,n_samples)
    Results$mu_a[i] <- mu_a <- update_mu(a,sig2_a,mu_a_mu,sigma2_mu_a,L=-10,U=10,n_samples)
    Results$mu_b[i] <- mu_b <- update_mu(b,sig2_b,mu_b_mu,sigma2_mu_b,L=-10,U=10,n_samples)
    ...
    Results$kappa_hk[i,,] <- kappa_hk <- update_kappa(Y_hk,a,b,phi,d_hk,lambda_hk,
                                                      sig2_e,sig2_kappa_hk,n_hk,n_samples)
    Results$kappa_reg[i,,] <- kappa_reg <- update_kappa(Y_reg,a,b,phi,d_reg,lambda_reg,
                                                        sig2_e,sig2_kappa_reg,n_reg,n_samples)
    
    for (j in 1:(n_neg-1)){
      Results$d_neg[i,j] <- d_neg[j] <- update_dneg2(Y_neg,cc,a,b,d_neg,sig2_dn,sig2_n,n_samples,n_neg,j)
    }
    Results$d_neg[i,n_neg] <-d_neg[n_neg] <- -1*sum(d_neg[-(n_neg)])
    
    Results$d_pos[i,] <- d_pos <- update_dpos3(Y_pos,X_pos,a,b,sig2_d,sig2_e,n_pos,n_samples,Sig_uns,D5,D6)
    
    Results$d_hk[i,] <- d_hk <- update_dhkreg(Y_hk,a,b,phi,kappa_hk,sig2_e,sig2_d,n_hk,n_samples)
    Results$d_reg[i,] <- d_reg <- update_dhkreg(Y_reg,a,b,phi,kappa_reg,sig2_e,sig2_d,n_reg,n_samples)
    Results$sig2_kappa_hk[i] <- sig2_kappa_hk <- update_sig2_kappa(kappa_hk,lambda_hk,n_hk,n_samples)
    Results$sig2_kappa_reg[i] <- sig2_kappa_reg <- update_sig2_kappa(kappa_reg,lambda_reg,n_reg,n_samples)
    Results$sig2_a[i] <- sig2_a <- update_sig2_abd(a,mu_a,n_samples)
    Results$sig2_b[i] <- sig2_b <- update_sig2_abd(b,mu_b,n_samples)
    Results$sig2_dn[i] <- sig2_dn <- update_sig2_abd(d_neg,0,n_neg)
    Results$sig2_d[i] <- sig2_d <- update_sig2_abd(d_pos,0,n_pos)
    Results$sig2_n[i] <- sig2_n <- update_sig2_n(Y_neg,a,b,cc,d_neg,n_neg,n_samples,eps=.01)
    Results$sig2_e[i] <- sig2_e <- update_sig2_e(Y_pos,X_pos,Y_hk,Y_reg,a,b,phi,kappa_hk,kappa_reg,
